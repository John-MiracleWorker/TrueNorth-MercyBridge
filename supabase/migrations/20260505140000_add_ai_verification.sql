-- Add AI verification fields to mercybridge_contributions
-- Enables automated payment proof verification with AI
...
COMMENT ON COLUMN mercybridge_contributions.ai_verification_status IS 'AI verification result: pending, verified (auto-approved), flagged (needs admin), failed (AI error)';
...
-- RLS: Allow admins to see all verification results
DROP POLICY IF EXISTS contributions_select_admin ON mercybridge_contributions;
...
-- RLS: Allow sponsors to see their own verification status
...
-- RLS: Allow system to update AI verification fields
-- (Handled via Edge Function with service role)
