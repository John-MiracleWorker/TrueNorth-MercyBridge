  ...
  return (
    <section className="rounded-2xl border border-white/10 bg-white/[0.055] shadow-xl shadow-black/15 backdrop-blur-xl">
      <div className="flex flex-col gap-3 border-b border-white/10 p-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div role="group" aria-labelledby="review-queue-heading" className="flex flex-wrap gap-2">
          {filters.map((item) => (
            <button
              key={item}
              aria-label={`Filter by ${item.replace('_', ' ')}`}
              aria-pressed={filter === item}
              className={`rounded-full border px-3 py-1 text-xs capitalize transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-amber-500/50 ${
                filter === item
                  ? 'border-amber-500/40 bg-amber-500/10 text-amber-300'
                  : 'border-white/10 text-slate-400 hover:bg-white/[0.04] hover:text-slate-300'
              }`}
